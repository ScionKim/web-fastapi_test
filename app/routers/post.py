...
async def create_post(post: schemas.PostCreate, db: Session=Depends(get_db), current_user: int=Depends(oauth2.get_current_user)):
    # q = "INSERT INTO posts (title, content, published) VALUES (%s, %s, %s) RETURNING *"
    # cursor.execute(q, (post.title, post.content, post.published))
...
async def delete_post(id: int, db: Session=Depends(get_db), current_user: int=Depends(oauth2.get_current_user)):
    # q = "DELETE FROM posts WHERE id = %s RETURNING *"
    # cursor.execute(q, (str(id),))
...
async def update_post(id: int, updated_post: schemas.PostCreate, db: Session=Depends(get_db), current_user: int=Depends(oauth2.get_current_user)):
    # q = "UPDATE posts SET title = %s, content = %s, published = %s WHERE id = %s RETURNING *"
    # cursor.execute(q, (post.title, post.content, post.published, str(id)))
